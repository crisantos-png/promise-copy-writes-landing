      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16 animate-fade-in">
          <h2 className="text-4xl md:text-5xl font-bold mb-6 text-gray-900 hover:scale-105 transition-transform duration-300">
            Services That <span className="text-blue-600 animate-pulse">Deliver Results</span>
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card 
              key={index} 
              className="border-0 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-4 bg-white group opacity-0 animate-fade-in"
              ...
              <CardContent>
                <ul className="space-y-2">
                  {service.features.map((feature, idx) => (
                    <li 
                      key={idx} 
                      className="flex items-center text-gray-700 hover:text-blue-600 transition-colors duration-300 transform hover:translate-x-2"
                    >
                      <div className="w-2 h-2 bg-blue-500 rounded-full mr-3 group-hover:bg-cyan-400 transition-colors duration-300 animate-pulse"></div>
                      {feature}
                    </li>
                  ))}
                </ul>
